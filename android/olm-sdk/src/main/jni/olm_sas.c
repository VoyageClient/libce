...
JNIEXPORT jlong OLM_SAS_FUNC_DEF(createNewSASJni)(JNIEnv *env, jobject thiz)
{
    ...
    LOGD("## createNewSASJni(): randomSize=%lu",(long unsigned int)(randomSize));
...
JNIEXPORT void OLM_SAS_FUNC_DEF(setTheirPubKey)(JNIEnv *env, jobject thiz,jbyteArray pubKeyBuffer) {
    ...
    else if (!(pubKeyPtr = (*env)->GetByteArrayElements(env, pubKeyBuffer, &pubKeyWasCopied)))
    {
    ...
    {
        size_t pubKeyLength = (size_t)(*env)->GetArrayLength(env, pubKeyBuffer);
    ...
    if (pubKeyPtr)
    {
        if (pubKeyWasCopied)
        {
            memset(pubKeyPtr, 0, (size_t)(*env)->GetArrayLength(env, pubKeyBuffer));
        }
        (*env)->ReleaseByteArrayElements(env, pubKeyBuffer, pubKeyPtr, JNI_ABORT);
    }
...
JNIEXPORT jbyteArray OLM_SAS_FUNC_DEF(generateShortCodeJni)(JNIEnv *env, jobject thiz, jbyteArray infoStringBytes, jint byteNb) {
    ...
    if (infoPtr)
    {
        if (infoWasCopied)
        {
            memset(infoPtr, 0, (size_t)(*env)->GetArrayLength(env, infoStringBytes));
        }
        (*env)->ReleaseByteArrayElements(env, infoStringBytes, infoPtr, JNI_ABORT);
    }
...
JNIEXPORT jbyteArray OLM_SAS_FUNC_DEF(calculateMacJni)(JNIEnv *env, jobject thiz,jbyteArray messageBuffer,jbyteArray infoBuffer) {
    ...
    else if (!(messagePtr = (*env)->GetByteArrayElements(env, messageBuffer, &messageWasCopied)))
    {
    ...
    else if (!(infoPtr = (*env)->GetByteArrayElements(env, infoBuffer, &infoWasCopied)))
    {
        ...
        size_t infoLength = (size_t)(*env)->GetArrayLength(env, infoBuffer);
        size_t messageLength = (size_t)(*env)->GetArrayLength(env, messageBuffer);
    ...
    if (infoPtr)
    {
        if (infoWasCopied)
        {
            memset(infoPtr, 0, (size_t)(*env)->GetArrayLength(env, infoBuffer));
        }
        (*env)->ReleaseByteArrayElements(env, infoBuffer, infoPtr, JNI_ABORT);
    }
    if (messagePtr)
    {
        if (messageWasCopied)
        {
            memset(messagePtr, 0, (size_t)(*env)->GetArrayLength(env, messageBuffer));
        }
        (*env)->ReleaseByteArrayElements(env, messageBuffer, messagePtr, JNI_ABORT);
    }
