...
OlmPkEncryption * initializePkEncryptionMemory()
{
    ...
    if (encryptionPtr)
    {
        ...
        LOGD(
            ...
            (long unsigned int)(encryptionSize)
        );
    }
    else
    {
...
JNIEXPORT void OLM_PK_ENCRYPTION_FUNC_DEF(setRecipientKeyJni)(
    ...
    else if (!(keyPtr = (*env)->GetByteArrayElements(env, aKeyBuffer, 0)))
    {
    ...
    {
        if (olm_pk_encryption_set_recipient_key(encryptionPtr, keyPtr, (size_t)(*env)->GetArrayLength(env, aKeyBuffer)) == olm_error())
        {
...
JNIEXPORT jbyteArray OLM_PK_ENCRYPTION_FUNC_DEF(encryptJni)(
    ...
    else if (!(ephemeralFieldId = (*env)->GetFieldID(env, encryptedMsgJClass, "mEphemeralKey", "Ljava/lang/String;")))
    {
    ...
    {
        size_t plaintextLength = (size_t)(*env)->GetArrayLength(env, aPlaintextBuffer);
        size_t ciphertextLength = olm_pk_ciphertext_length(plaintextLength);
        size_t macLength = olm_pk_mac_length();
        size_t ephemeralLength = olm_pk_key_length();
        uint8_t *ciphertextPtr = NULL, *macPtr = NULL, *ephemeralPtr = NULL;
        size_t randomLength = olm_pk_encrypt_random_length();
        uint8_t *randomBuffPtr = NULL;
        LOGD("## pkEncryptJni(): randomLength=%lu",(long unsigned int)(randomLength));
        ...
        else
        {
            ...
            {
                encryptedMsgRet = (*env)->NewByteArray(env, ciphertextLength);
                (*env)->SetByteArrayRegion(env, 
                    encryptedMsgRet, 0, ciphertextLength, (jbyte*)ciphertextPtr
                );

                jstring macStr = (*env)->NewStringUTF(env, (char*)macPtr);
                (*env)->SetObjectField(env, aEncryptedMsg, macFieldId, macStr);
                jstring ephemeralStr = (*env)->NewStringUTF(env, (char*)ephemeralPtr);
                (*env)->SetObjectField(env, aEncryptedMsg, ephemeralFieldId, ephemeralStr);
            }
        }
    ...
    if (plaintextPtr)
    {
        ...
        {
            memset(plaintextPtr, 0, (size_t)(*env)->GetArrayLength(env, aPlaintextBuffer));
        }
        (*env)->ReleaseByteArrayElements(env, aPlaintextBuffer, plaintextPtr, JNI_ABORT);
    }
...
OlmPkDecryption * initializePkDecryptionMemory()
{
    ...
    if (decryptionPtr)
    {
        ...
        LOGD(
            ...
            (long unsigned int)(decryptionSize)
        );
    }
    else
    {
...
JNIEXPORT jbyteArray OLM_PK_DECRYPTION_FUNC_DEF(setPrivateKeyJni)(JNIEnv *env, jobject thiz, jbyteArray key)
{
    ...
    else if (!(keyPtr = (*env)->GetByteArrayElements(env, key, &keyWasCopied)))
    {
    ...
    else
    {
        ...
        size_t keyLength = (size_t)(*env)->GetArrayLength(env, key);
    ...
    if (keyPtr)
    {
        if (keyWasCopied)
        {
            memset(keyPtr, 0, (size_t)(*env)->GetArrayLength(env, key));
        }
...
JNIEXPORT jbyteArray OLM_PK_DECRYPTION_FUNC_DEF(generateKeyJni)(JNIEnv *env, jobject thiz)
{
    ...
    else if (!(publicKeyPtr = (uint8_t*)(malloc(publicKeyLength))))
    {
...
JNIEXPORT jbyteArray OLM_PK_DECRYPTION_FUNC_DEF(decryptJni)(
    ...
    else if (!(ciphertextFieldId = (*env)->GetFieldID(env, encryptedMsgJClass,"mCipherText","Ljava/lang/String;")))
    {
    ...
    else if (!(macJstring = (jstring)(*env)->GetObjectField(env, aEncryptedMsg, macFieldId)))
    {
        LOGE("## pkDecryptJni(): failure - no MAC");
        errorMessage = "no MAC";
    }
    else if (!(macPtr = (*env)->GetStringUTFChars(env, macJstring, 0)))
    {
        LOGE("## pkDecryptJni(): failure - MAC JNI allocation OOM");
        errorMessage = "ciphertext JNI allocation OOM";
    }
    else if (!(ephemeralKeyFieldId = (*env)->GetFieldID(env, encryptedMsgJClass,"mEphemeralKey","Ljava/lang/String;")))
    {
        LOGE("## pkDecryptJni(): failure - unable to get ephemeral key field");
        errorMessage = "unable to get ephemeral key field";
    }
    else if (!(ephemeralKeyJstring = (jstring)(*env)->GetObjectField(env, aEncryptedMsg, ephemeralKeyFieldId)))
    {
        LOGE("## pkDecryptJni(): failure - no ephemeral key");
        errorMessage = "no ephemeral key";
    }
    else if (!(ephemeralKeyPtr = (*env)->GetStringUTFChars(env, ephemeralKeyJstring, 0)))
    {
    ...
    else
    {
        size_t maxPlaintextLength = olm_pk_max_plaintext_length(
            (size_t)(*env)->GetStringUTFLength(env, ciphertextJstring)
        );
        uint8_t *plaintextPtr = NULL;
        uint8_t *tempCiphertextPtr = NULL;
        size_t ciphertextLength = (size_t)(*env)->GetStringUTFLength(env, ciphertextJstring);
        ...
        else
        {
            ...
            size_t plaintextLength = olm_pk_decrypt(
                decryptionPtr,
                ephemeralKeyPtr, (size_t)(*env)->GetStringUTFLength(env, ephemeralKeyJstring),
                macPtr, (size_t)(*env)->GetStringUTFLength(env, macJstring),
            ...
            else
            {
                ...
                (*env)->SetByteArrayRegion(env, decryptedMsgRet, 0, plaintextLength, (jbyte*)plaintextPtr);
                LOGD(
                    "## pkDecryptJni(): success returnedLg=%lu OK",
                    (long unsigned int)(plaintextLength)
                );
            }
        }
    ...
    if (ciphertextPtr)
    {
        (*env)->ReleaseStringUTFChars(env, ciphertextJstring, ciphertextPtr);
    }
    if (macPtr)
    {
        (*env)->ReleaseStringUTFChars(env, macJstring, macPtr);
    }
    if (ephemeralKeyPtr)
    {
        (*env)->ReleaseStringUTFChars(env, ephemeralKeyJstring, ephemeralKeyPtr);
    }
...
JNIEXPORT jbyteArray OLM_PK_SIGNING_FUNC_DEF(generateSeedJni)(JNIEnv *env, jobject thiz)
{
    ...
    else if (!(randomRet = (*env)->NewByteArray(env, randomLength)))
    {
...
JNIEXPORT jbyteArray OLM_PK_SIGNING_FUNC_DEF(setKeyFromSeedJni)(JNIEnv *env, jobject thiz, jbyteArray seed)
{
    ...
    else if (!(seedPtr = (*env)->GetByteArrayElements(env, seed, &seedWasCopied)))
    {
    ...
    else
    {
        ...
        uint8_t *publicKeyPtr = NULL;
        size_t seedLength = (size_t)(*env)->GetArrayLength(env, seed);
        if (!(publicKeyPtr = (uint8_t*)malloc(publicKeyLength)))
        {
            errorMessage = "public key JNI allocation OOM";
        ...
        else
        {
            ...
            {
                if (!(publicKeyRet = (*env)->NewByteArray(env, publicKeyLength))) {
    ...
    if (seedPtr)
    {
        if (seedWasCopied)
        {
            memset(seedPtr, 0, (size_t)(*env)->GetArrayLength(env, seed));
        }
        (*env)->ReleaseByteArrayElements(env, seed, seedPtr, JNI_ABORT);
    }
...
JNIEXPORT jbyteArray OLM_PK_SIGNING_FUNC_DEF(pkSignJni)(JNIEnv *env, jobject thiz, jbyteArray aMessage)
{
    ...
    else if (!(messagePtr = (*env)->GetByteArrayElements(env, aMessage, &messageWasCopied)))
    {
    ...
    else
    {
        ...
        size_t messageLength = (size_t)(*env)->GetArrayLength(env, aMessage);
        ...
        else
        {
            ...
            {
                if (!(signatureRet = (*env)->NewByteArray(env, signatureLength))) {
    ...
    if (messagePtr)
    {
        if (messageWasCopied)
        {
            memset(messagePtr, 0, (size_t)(*env)->GetArrayLength(env, aMessage));
        }
        (*env)->ReleaseByteArrayElements(env, aMessage, messagePtr, JNI_ABORT);
    }
