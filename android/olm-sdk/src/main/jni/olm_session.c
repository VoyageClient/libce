...
OlmSession* initializeSessionMemory()
{
    ...
    if (sessionPtr)
    {
        ...
        LOGD("## initializeSessionMemory(): success - OLM session size=%lu",(long unsigned int)(sessionSize));
    }
    else
    {
...
JNIEXPORT jlong OLM_SESSION_FUNC_DEF(createNewSessionJni)(JNIEnv *env, jobject thiz)
{
    ...
    if (!accountPtr)
    {
        ...
        (*env)->ThrowNew(env, (*env)->FindClass(env, "java/lang/Exception"), "init session OOM");
    }
    else
    {
...
JNIEXPORT void OLM_SESSION_FUNC_DEF(initOutboundSessionJni)(JNIEnv *env, jobject thiz, jlong aOlmAccountId, jbyteArray aTheirIdentityKeyBuffer, jbyteArray aTheirOneTimeKeyBuffer)
{
    ...
    else
    {
        ...
        LOGD("## initOutboundSessionJni(): randomSize=%lu",(long unsigned int)(randomSize));
        ...
        else
        {
            ...
            if (!(theirIdentityKeyPtr = (*env)->GetByteArrayElements(env, aTheirIdentityKeyBuffer, 0)))
            {
                LOGE("## initOutboundSessionJni(): failure - identityKey JNI allocation OOM");
                errorMessage = "identityKey JNI allocation OOM";
            }
            else if (!(theirOneTimeKeyPtr = (*env)->GetByteArrayElements(env, aTheirOneTimeKeyBuffer, 0)))
            {
            ...
            {
                size_t theirIdentityKeyLength = (size_t)(*env)->GetArrayLength(env, aTheirIdentityKeyBuffer);
                size_t theirOneTimeKeyLength  = (size_t)(*env)->GetArrayLength(env, aTheirOneTimeKeyBuffer);
                LOGD("## initOutboundSessionJni(): identityKey=%.*s oneTimeKey=%.*s", (int)(theirIdentityKeyLength), theirIdentityKeyPtr, (int)(theirOneTimeKeyLength), theirOneTimeKeyPtr);
                ...
                {
                    LOGD("## initOutboundSessionJni(): success - result=%lu", (long unsigned int)(sessionResult));
                }
            }

            if (theirIdentityKeyPtr)
            {
                (*env)->ReleaseByteArrayElements(env, aTheirIdentityKeyBuffer, theirIdentityKeyPtr, JNI_ABORT);
            }

            if (theirOneTimeKeyPtr)
            {
                (*env)->ReleaseByteArrayElements(env, aTheirOneTimeKeyBuffer, theirOneTimeKeyPtr, JNI_ABORT);
            }
...
JNIEXPORT void OLM_SESSION_FUNC_DEF(initInboundSessionJni)(JNIEnv *env, jobject thiz, jlong aOlmAccountId, jbyteArray aOneTimeKeyMsgBuffer)
{
    ...
    {
        jbyte* messagePtr = (*env)->GetByteArrayElements(env, aOneTimeKeyMsgBuffer, 0);
        ...
        {
            size_t messageLength = (size_t)(*env)->GetArrayLength(env, aOneTimeKeyMsgBuffer);
            LOGD("## initInboundSessionJni(): messageLength=%lu message=%.*s", (long unsigned int)(messageLength), (int)(messageLength), messagePtr);
            ...
            {
                LOGD("## initInboundSessionJni(): success - result=%lu", (long unsigned int)(sessionResult));
            }

            // free local alloc
            (*env)->ReleaseByteArrayElements(env, aOneTimeKeyMsgBuffer, messagePtr, JNI_ABORT);
        }
    }
...
JNIEXPORT void OLM_SESSION_FUNC_DEF(initInboundSessionFromIdKeyJni)(JNIEnv *env, jobject thiz, jlong aOlmAccountId, jbyteArray aTheirIdentityKeyBuffer, jbyteArray aOneTimeKeyMsgBuffer)
{
    ...
    else if (!(messagePtr = (*env)->GetByteArrayElements(env, aOneTimeKeyMsgBuffer, 0)))
    {
        LOGE("## initInboundSessionFromIdKeyJni(): failure - message JNI allocation OOM");
        errorMessage = "message JNI allocation OOM";
    }
    else if(!(theirIdentityKeyPtr = (*env)->GetByteArrayElements(env, aTheirIdentityKeyBuffer, 0)))
    {
    ...
    else
    {
        ...
        LOGD("## initInboundSessionFromIdKeyJni(): message=%.*s messageLength=%lu", (int)(messageLength), messagePtr, (long unsigned int)(messageLength));
        ...
        {
            LOGD("## initInboundSessionFromIdKeyJni(): success - result=%lu", (long unsigned int)(sessionResult));
        }
     }
...
JNIEXPORT jboolean OLM_SESSION_FUNC_DEF(matchesInboundSessionJni)(JNIEnv *env, jobject thiz, jbyteArray aOneTimeKeyMsgBuffer)
{
    ...
    else
    {
        ...
        {
            retCode = JNI_TRUE;
            LOGD("## matchesInboundSessionJni(): success - result=%lu", (long unsigned int)(matchResult));
        }
    }
...
JNIEXPORT jboolean JNICALL OLM_SESSION_FUNC_DEF(matchesInboundSessionFromIdKeyJni)(JNIEnv *env, jobject thiz, jbyteArray aTheirIdentityKeyBuffer, jbyteArray aOneTimeKeyMsgBuffer)
{
    ...
    {
        size_t identityKeyLength = (size_t)(*env)->GetArrayLength(env, aTheirIdentityKeyBuffer);
        size_t messageLength = (size_t)(*env)->GetArrayLength(env, aOneTimeKeyMsgBuffer);
        ...
        {
            retCode = JNI_TRUE;
            LOGD("## matchesInboundSessionFromIdKeyJni(): success - result=%lu", (long unsigned int)(matchResult));
        }
    }
...
JNIEXPORT jbyteArray OLM_SESSION_FUNC_DEF(encryptMessageJni)(JNIEnv *env, jobject thiz, jbyteArray aClearMsgBuffer, jobject aEncryptedMsg)
{
    ...
    else if (!(clearMsgPtr = (*env)->GetByteArrayElements(env, aClearMsgBuffer, &clearMsgIsCopied)))
    {
        LOGE("## encryptMessageJni(): failure - clear message JNI allocation OOM");
        errorMessage = "clear message JNI allocation OOM";
    }
    else if (!(encryptedMsgJClass = (*env)->GetObjectClass(env, aEncryptedMsg)))
    {
        LOGE("## encryptMessageJni(): failure - unable to get crypted message class");
        errorMessage = "unable to get crypted message class";
    }
    else if (!(typeMsgFieldId = (*env)->GetFieldID(env, encryptedMsgJClass,"mType","J")))
    {
    ...
    else
    {
        ...
        LOGD("## encryptMessageJni(): randomLength=%lu", (long unsigned int)(randomLength));
        ...
        else
        {
            ...
            size_t clearMsgLength = (size_t)(*env)->GetArrayLength(env, aClearMsgBuffer);
            ...
            else
            {
                ...
                LOGD("## encryptMessageJni(): messageType=%lu randomLength=%lu clearMsgLength=%lu encryptedMsgLength=%lu",(long unsigned int)(messageType),(long unsigned int)(randomLength), (long unsigned int)(clearMsgLength), (long unsigned int)(encryptedMsgLength));
                // encrypt message
                ...
                else
                {
                    ...
                    (*env)->SetLongField(env, aEncryptedMsg, typeMsgFieldId, (jlong)messageType);

                    encryptedMsgRet = (*env)->NewByteArray(env, encryptedMsgLength);
                    (*env)->SetByteArrayRegion(env, encryptedMsgRet, 0 , encryptedMsgLength, (jbyte*)encryptedMsgPtr);

                    LOGD("## encryptMessageJni(): success - result=%lu Type=%lu encryptedMsg=%.*s", (long unsigned int)(result), (unsigned long int)(messageType), (int)(result), (const char*)encryptedMsgPtr);
                }
    ...
    if (clearMsgPtr)
    {
        if (clearMsgIsCopied)
        {
            memset(clearMsgPtr, 0, (size_t)(*env)->GetArrayLength(env, aClearMsgBuffer));
        }
        (*env)->ReleaseByteArrayElements(env, aClearMsgBuffer, clearMsgPtr, JNI_ABORT);
    }
...
JNIEXPORT jbyteArray OLM_SESSION_FUNC_DEF(decryptMessageJni)(JNIEnv *env, jobject thiz, jobject aEncryptedMsg)
{
    ...
    else if (!(encryptedMsgFieldId = (*env)->GetFieldID(env, encryptedMsgJClass,"mCipherText","Ljava/lang/String;")))
    {
    ...
    else if (!(encryptedMsgJstring = (jstring)(*env)->GetObjectField(env, aEncryptedMsg, encryptedMsgFieldId)))
    {
        LOGE("## decryptMessageJni(): failure - JNI encrypted object ");
        errorMessage = "JNI encrypted object";
    }
    else if (!(encryptedMsgPtr = (*env)->GetStringUTFChars(env, encryptedMsgJstring, 0)))
    {
    ...
    {
        // get message type
        size_t encryptedMsgType = (size_t)(*env)->GetLongField(env, aEncryptedMsg, typeMsgFieldId);
        // get encrypted message length
        size_t encryptedMsgLength = (size_t)(*env)->GetStringUTFLength(env, encryptedMsgJstring);
        ...
        else
        {
            ...
            {
                decryptedMsgRet = (*env)->NewByteArray(env, plaintextLength);
                (*env)->SetByteArrayRegion(env, decryptedMsgRet, 0 , plaintextLength, (jbyte*)plainTextMsgPtr);
    ...
    if (encryptedMsgPtr)
    {
        (*env)->ReleaseStringUTFChars(env, encryptedMsgJstring, encryptedMsgPtr);
    }
...
JNIEXPORT jbyteArray OLM_SESSION_FUNC_DEF(getSessionIdentifierJni)(JNIEnv *env, jobject thiz)
{
     ...
     else
     {
         ...
         LOGD("## getSessionIdentifierJni(): lengthSessionId=%lu",(long unsigned int)(lengthSessionId));
         ...
         else
         {
             ...
             {
                 LOGD("## getSessionIdentifierJni(): success - result=%lu sessionId=%.*s",(long unsigned int)(result), (int)(result), (char*)sessionIdPtr);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                returnValue = (*env)->NewByteArray(env, pickledLength);
                (*env)->SetByteArrayRegion(env, returnValue, 0 , pickledLength, (jbyte*)pickledPtr);
            }
...
JNIEXPORT jlong OLM_SESSION_FUNC_DEF(deserializeJni)(JNIEnv *env, jobject thiz, jbyteArray aSerializedDataBuffer, jbyteArray aKeyBuffer)
{
    ...
    else
    {
        ...
        {
            LOGD(" ## initJni(): success - result=%lu ", (long unsigned int)(result));
        }
    }
