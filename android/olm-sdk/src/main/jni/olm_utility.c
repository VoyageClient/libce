...
OlmUtility* initializeUtilityMemory()
{
    ...
    if (utilityPtr)
    {
        ...
        LOGD("## initializeUtilityMemory(): success - OLM utility size=%lu",(long unsigned int)(utilitySize));
    }
    else
    {
...
JNIEXPORT jlong OLM_UTILITY_FUNC_DEF(createUtilityJni)(JNIEnv *env, jobject thiz)
{
    ...
    if (!utilityPtr)
    {
        ...
        (*env)->ThrowNew(env, (*env)->FindClass(env, "java/lang/Exception"), "init OOM");
    }
    else
    {
...
JNIEXPORT jstring OLM_UTILITY_FUNC_DEF(verifyEd25519SignatureJni)(JNIEnv *env, jobject thiz, jbyteArray aSignatureBuffer, jbyteArray aKeyBuffer, jbyteArray aMessageBuffer)
{
    ...
    else if (!(signaturePtr = (*env)->GetByteArrayElements(env, aSignatureBuffer, 0)))
    {
    ...
    else if (!(messagePtr = (*env)->GetByteArrayElements(env, aMessageBuffer, &messageWasCopied)))
    {
        LOGE(" ## verifyEd25519SignatureJni(): failure - message JNI allocation OOM");
    }
    else
    {
        size_t signatureLength = (size_t)(*env)->GetArrayLength(env, aSignatureBuffer);
        size_t keyLength = (size_t)(*env)->GetArrayLength(env, aKeyBuffer);
        size_t messageLength = (size_t)(*env)->GetArrayLength(env, aMessageBuffer);
        LOGD(" ## verifyEd25519SignatureJni(): signatureLength=%lu keyLength=%lu messageLength=%lu",(long unsigned int)(signatureLength),(long unsigned int)(keyLength),(long unsigned int)(messageLength));
        LOGD(" ## verifyEd25519SignatureJni(): key=%.*s", (int)(keyLength), keyPtr);
        ...
        if (result == olm_error()) {
            ...
            errorMessageRetValue = (*env)->NewStringUTF(env, errorMsgPtr);
            LOGE("## verifyEd25519SignatureJni(): failure - olm_ed25519_verify Msg=%s",errorMsgPtr);
        }
        else
        {
            LOGD("## verifyEd25519SignatureJni(): success - result=%lu", (long unsigned int)(result));
        }
    }
    ...
    if (signaturePtr)
    {
        (*env)->ReleaseByteArrayElements(env, aSignatureBuffer, signaturePtr, JNI_ABORT);
    }
    ...
    {
        if (messageWasCopied) {
            memset(messagePtr, 0, (size_t)(*env)->GetArrayLength(env, aMessageBuffer));
        }
        (*env)->ReleaseByteArrayElements(env, aMessageBuffer, messagePtr, JNI_ABORT);
    }
...
JNIEXPORT jbyteArray OLM_UTILITY_FUNC_DEF(sha256Jni)(JNIEnv *env, jobject thiz, jbyteArray aMessageToHashBuffer)
{
    ...
    else if(!(messagePtr = (*env)->GetByteArrayElements(env, aMessageToHashBuffer, &messageWasCopied)))
    {
    ...
    {
        // get lengths
        size_t messageLength = (size_t)(*env)->GetArrayLength(env, aMessageToHashBuffer);
        size_t hashLength = olm_sha256_length();
        ...
        else
        {
            ...
            {
                LOGD("## sha256Jni(): success - result=%lu hashValue=%.*s",(long unsigned int)(result), (int)(result), (char*)hashValuePtr);
                sha256Ret = (*env)->NewByteArray(env, result);
                (*env)->SetByteArrayRegion(env, sha256Ret, 0 , result, (jbyte*)hashValuePtr);
            }
    ...
    {
        if (messageWasCopied) {
            memset(messagePtr, 0, (size_t)(*env)->GetArrayLength(env, aMessageToHashBuffer));
        }
        (*env)->ReleaseByteArrayElements(env, aMessageToHashBuffer, messagePtr, JNI_ABORT);
    }
