...
JNIEXPORT void OLM_OUTBOUND_GROUP_SESSION_FUNC_DEF(releaseSessionJni)(JNIEnv *env, jobject thiz)
{
    ...
    else
    {
        ...
        LOGD(" ## releaseSessionJni(): clear_outbound_group_session=%lu",(long unsigned int)(retCode));
#else
...
JNIEXPORT jlong OLM_OUTBOUND_GROUP_SESSION_FUNC_DEF(createNewSessionJni)(JNIEnv *env, jobject thiz)
{
    ...
    else
    {
        ...
        LOGD(" ## createNewSessionJni(): success - outbound group session size=%lu",(long unsigned int)(sessionSize));

        // compute random buffer
        size_t randomLength = olm_init_outbound_group_session_random_length(sessionPtr);
        uint8_t *randomBuffPtr = NULL;

        LOGW(" ## createNewSessionJni(): randomLength=%lu",(long unsigned int)(randomLength));
...
JNIEXPORT jbyteArray OLM_OUTBOUND_GROUP_SESSION_FUNC_DEF(sessionIdentifierJni)(JNIEnv *env, jobject thiz)
{
    ...
    else
    {
        ...
        LOGD(" ## sessionIdentifierJni(): outbound group session lengthSessionId=%lu",(long unsigned int)(lengthSessionId));
        ...
        else
        {
            ...
            else
            {
                ...
                LOGD(" ## sessionIdentifierJni(): success - outbound group session identifier result=%lu sessionId= %.*s",(long unsigned int)(result), (int)(result), (char*)(sessionIdPtr));
            }
...
JNIEXPORT jint OLM_OUTBOUND_GROUP_SESSION_FUNC_DEF(messageIndexJni)(JNIEnv *env, jobject thiz)
{
    ...
    {
        indexRetValue = (jint)(olm_outbound_group_session_message_index(sessionPtr));
    }
...
JNIEXPORT jbyteArray OLM_OUTBOUND_GROUP_SESSION_FUNC_DEF(sessionKeyJni)(JNIEnv *env, jobject thiz)
{
    ...
    else
    {
        ...
        LOGD(" ## sessionKeyJni(): sessionKeyLength=%lu",(long unsigned int)(sessionKeyLength));
        ...
        else
        {
            ...
            {
                LOGD(" ## sessionKeyJni(): success - outbound group session key result=%lu sessionKey=%.*s",(long unsigned int)(result), (int)(result), (char*)(sessionKeyPtr));

                returnValue = (*env)->NewByteArray(env, result);
                (*env)->SetByteArrayRegion(env, returnValue, 0 , result, (jbyte*)sessionKeyPtr);
            }
...
JNIEXPORT jbyteArray OLM_OUTBOUND_GROUP_SESSION_FUNC_DEF(encryptMessageJni)(JNIEnv *env, jobject thiz, jbyteArray aClearMsgBuffer)
{
    ...
    else
    {
        ...
        LOGD(" ## encryptMessageJni(): clearMsgLength=%lu",(long unsigned int)(clearMsgLength));
        ...
        {
            LOGD(" ## encryptMessageJni(): estimated encryptedMsgLength=%lu",(long unsigned int)(encryptedMsgLength));
            ...
            {
                LOGD(" ## encryptMessageJni(): encrypted returnedLg=%lu plainTextMsgPtr=%.*s",(long unsigned int)(encryptedLength), (int)(encryptedLength), (char*)(encryptedMsgPtr));

                encryptedMsgRet = (*env)->NewByteArray(env, encryptedLength);
                (*env)->SetByteArrayRegion(env, encryptedMsgRet, 0 , encryptedLength, (jbyte*)encryptedMsgPtr);
            }
...
JNIEXPORT jbyteArray OLM_OUTBOUND_GROUP_SESSION_FUNC_DEF(serializeJni)(JNIEnv *env, jobject thiz, jbyteArray aKeyBuffer)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                LOGD(" ## serializeJni(): success - result=%lu pickled=%.*s", (long unsigned int)(result), (int)(result), (char*)(pickledPtr));
