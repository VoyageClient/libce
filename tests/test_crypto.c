...
static void test_curve25519_case_1(void **state)
{
    (void)state;
    ...
    uint8_t actual_agreement[CURVE25519_SHARED_SECRET_LENGTH] = {0};
...
static void test_ed25519_signature_case_1(void **state)
{
    (void)state;

    uint8_t private_key[33] = "This key is a string of 32 bytes";

    uint8_t message[] = "Hello, World";
    size_t message_length = sizeof(message) - 1U;

    _olm_ed25519_key_pair key_pair;
    _olm_crypto_ed25519_generate_key(private_key, &key_pair);

    uint8_t signature[64];
    _olm_crypto_ed25519_sign(&key_pair, message, message_length, signature);

    int result = _olm_crypto_ed25519_verify(
        &key_pair.public_key,
        message,
        message_length,
        signature
...
static void test_aes_case_1(void **state)
{
    (void)state;

    _olm_aes256_key key = {0};
    _olm_aes256_iv iv = {0};
    uint8_t input[16] = {0};
    ...
    size_t length = _olm_crypto_aes_encrypt_cbc_length(sizeof(input));
    CHECK_EQ((size_t)32U, length);
    ...
    CHECK_EQ((size_t)16U, length);
    CHECK_EQ_SIZE(input, actual, length);
}

static void test_sha256_case_1(void **state)
{
    (void)state;
...
static void test_hmac_case_1(void **state)
{
    (void)state;
...
static void test_hkdf_case_1(void **state)
{
    (void)state;
    ...
    uint8_t hmac_expected_output[32] = {
    ...
    uint8_t hmac_actual_output[32] = {0};
    ...
    uint8_t hkdf_expected_output[42] = {
    ...
    uint8_t hkdf_actual_output[42] = {0};
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(test_curve25519_case_1),
        cmocka_unit_test(test_ed25519_signature_case_1),
        cmocka_unit_test(test_aes_case_1),
        cmocka_unit_test(test_sha256_case_1),
        cmocka_unit_test(test_hmac_case_1),
        cmocka_unit_test(test_hkdf_case_1),
    };
